...
class Pestplaces extends Component {
    ...
    async componentDidMount(){
        if(this.props.cityid){
            await axios.get(`${backendurl}/places?city=${this.props.cityid}&tag=${this.props.name}`)
                .then(res=>{ console.log(res); this.setState({places:res.data.places})})
        }
        else
        await axios.get(`${backendurl}/cities?tag=${this.props.name}`)
        .then(res=>{ this.setState({places:res.data.cities})})
